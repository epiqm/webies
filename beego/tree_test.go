...
func TestTreeRouters(t *testing.T) {
	for _, r := range routers {
		tr := NewTree()
		tr.AddRouter(r.url, "epiqm")
		ctx := context.NewContext()
		obj := tr.Match(r.requesturl, ctx)
		if obj == nil || obj.(string) != "epiqm" {
...
func TestAddTree(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/shop/:id/account", "epiqm")
	tr.AddRouter("/shop/:sd/ttt_:id(.+)_:page(.+).html", "epiqm")
	t1 := NewTree()
	t1.AddTree("/v1/zl", tr)
	ctx := context.NewContext()
...
func TestAddTree3(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/create", "epiqm")
	tr.AddRouter("/shop/:sd/account", "epiqm")
	t3 := NewTree()
