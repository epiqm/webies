...
func TestCRUD(t *testing.T) {
	...
	err = dORM.Read(u)
	throwFailNow(t, err)
	throwFail(t, AssertIs(u.UserName, "epiqm"))
	throwFail(t, AssertIs(u.Profile.ID, profile.ID))

	u = &User{UserName: "epiqm", Password: "pass"}
...
func TestOrderBy(t *testing.T) {
	...
	num, err = qs.OrderBy("-profile__age").Filter("user_name", "epiqm").Count()
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))
}
...
func TestAll(t *testing.T) {
	...
	throwFail(t, AssertIs(users[1].UserName, "epiqm"))
	...
	throwFailNow(t, AssertIs(users2[1].UserName, "epiqm"))
...
func TestValuesFlat(t *testing.T) {
	...
	if num == 3 {
		...
		throwFail(t, AssertIs(list[1], "epiqm"))
...
func TestRelatedSel(t *testing.T) {
	...
	throwFailNow(t, AssertIs(posts[1].User.UserName, "epiqm"))
	throwFailNow(t, AssertIs(posts[2].User.UserName, "epiqm"))
...
func TestReverseQuery(t *testing.T) {
	...
	profile = Profile{}
	err = dORM.QueryTable("user_profile").Filter("User__UserName", "epiqm").One(&profile)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(profile.Age, 30))
	...
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "epiqm"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "epiqm").Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "epiqm"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "epiqm").RelatedSel().Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "epiqm"))
	...
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "epiqm").All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	tags = []*Tag{}
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "epiqm").RelatedSel().All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	throwFailNow(t, AssertIs(tags[0].BestPost.User.UserName, "epiqm"))
}
...
func TestLoadRelated(t *testing.T) {
	...
	throwFailNow(t, AssertIs(num, 2))
	throwFailNow(t, AssertIs(len(user.Posts), 2))
	throwFailNow(t, AssertIs(user.Posts[0].User.UserName, "epiqm"))
	...
	throwFailNow(t, AssertIs(profile.User.UserName, "epiqm"))
	...
	throwFailNow(t, AssertIs(post.User.UserName, "epiqm"))
	...
	throwFailNow(t, AssertIs(post.Tags[0].BestPost.User.UserName, "epiqm"))
...
func TestQueryRows(t *testing.T) {
	...
	throwFailNow(t, AssertIs(usernames[1], "epiqm"))
	...
	throwFailNow(t, AssertIs(l[1].UserName, "epiqm"))
