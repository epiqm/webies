...
func TestMail(t *testing.T) {
	config := `{"username":"epiqm@gmail.com","password":"epiqm","host":"smtp.gmail.com","port":587}`
	mail := NewEMail(config)
	if mail.Username != "epiqm@gmail.com" {
		t.Fatal("email parse get username error")
	}
	if mail.Password != "epiqm" {
