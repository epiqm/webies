...
func TestUrlFor2(t *testing.T) {
	...
	if handler.URLFor("TestController.GetURL", ":username", "epiqm") != "/v1/epiqm/edit" {
		logs.Info(handler.URLFor("TestController.GetURL"))
		t.Errorf("TestController.List must equal to /v1/epiqm/edit")
	}
...
func TestPostFunc(t *testing.T) {
	r, _ := http.NewRequest("POST", "/epiqm", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/:name", &TestController{})
	handler.ServeHTTP(w, r)
	if w.Body.String() != "epiqm" {
		t.Errorf("post func should epiqm")
	}
}
